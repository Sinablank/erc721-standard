...
async function LoadDataSmartContract() {
    ...
    $(document).on('click', '#َApprove', approve);
...
async function approve() {
    ...
    var TokenID = $("#َTokenIDApprove").val();
    var AddressTO = $("#AdressToApprove").val();
    if (TokenID.trim() == '' || AddressTO.trim() == '') {
        alert("Please Fill TextBox");
    }

    MyContract.methods.approve(AddressTO, TokenID).send({ from: CurrentAccount }).then(function (Instance) {

        alert(Instance.events.Approval.returnValues[0] + " Approved TokenId " +
...
function mint() {
    ...
    MyContract.methods.mint(CurrentAccount, IPFS_Hash).send({ from: CurrentAccount }).then(function (Instance) {

        alert("NFT minted By " + Instance.events.Transfer.returnValues[1] + " With TokenId " +
            Instance.events.Transfer.returnValues[2]);
        //  window.location.reload();
    }).catch(function (error) {
        alert(error.message);
